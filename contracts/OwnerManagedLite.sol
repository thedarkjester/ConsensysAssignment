...
contract OwnerManagedLite {
    ...
    function withdraw() public { 
        ...
        emit LogDepositWithdrawn(msg.sender, balanceToSend); 
    }
    ...
    function distributeUnallocatedFunds() public callerIsStateOwner() { 
        ...
        for(uint i=0; i < state.owners.length; i++) {
          allocatedBalances = allocatedBalances.add(ownerBalances[state.owners[i]]);
        }

        uint256 amountToSplit = address(this).balance.sub(allocatedBalances);

        distributeFunds(amountToSplit, true);
     ...
     function distributeFunds(uint256 amount, bool callerIsOwner) internal { 
        uint256 split = amount.div(state.owners.length);
        uint256 remainder = amount.mod(state.owners.length);

        for(uint i=0; i < state.owners.length;i++) {
            ownerBalances[state.owners[i]] = ownerBalances[state.owners[i]].add(split);

            if(callerIsOwner) {
                if(state.owners[i] == msg.sender) {
                  ownerBalances[state.owners[i]] = ownerBalances[state.owners[i]].add(remainder);
                }
            }           
            else {
                if(i == (state.owners.length-1)) {
                    ownerBalances[state.owners[i]] = ownerBalances[state.owners[i]].add(remainder);
                }
            }
        }
